...
Если у объекта есть метод `toString`, он вызывается и результатом приведения будет то, что вернет этот метод. В противном случае произойдет ошибка. Однако в большинстве случаев у объектов автоматически доступен метод `toString`, результатом работы которого будет строка `"[object Object]"`, где `Object` соответствует классу объекта. (см [Объекты](objects.md) и [Классы](classes.md)). 
